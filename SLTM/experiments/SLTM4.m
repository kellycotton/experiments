...
%INFO%
%%%%%%
%Kelly Cotton 4/12/2019

%SLTM4: Surprise Long Term Memory 
...
%The stimuli in this experiment are words and pronounceable nonwords.
...
%Subj.Number | Block Number |  Trial Number | Cue Condition | Word Condition | Response RT | Response Position | Target Position | Accuracy
...
%Word conditions: 1 (word), 2 (nonword)
...
%Subj. Number | Block Number | Trial Number | Cue Condition | Word Condition | Decision | Accuracy | Response | Response Reaction Time

%Cue conditions: 1 (before), 2 (simultaneous), 0 (lure trial - two distractors)
%Word conditions: 1 (word), 2 (nonword), 0 (lure trial)
...
%[window,sRect]=Screen('OpenWindow', screenNumber, [], [0 0 640 480]);
...
%Restrict Input
RestrictKeysForKbCheck([KbName('control'),KbName('alt'),KbName('esc'),KbName('space'),KbName('a'),KbName('s'),KbName('z'),KbName('x'),KbName('7')]);
...
%Lists of nonwords to be presented
NWordlist=fopen('Nonwords.txt','r'); %Open the text file 
NWordlist=textscan(NWordlist,'%s'); %Reads the data from the file into a cell array
NWordlist=cell2mat(NWordlist); %Convert cell array into matrix
targetNwords={}; %Create an array to fill with the words used to be later used in the long term memory test
usedNWords=[]; %Create an array to fill with the words that were used because they will later be removed

%Randomize words 
Wordlist=Shuffle(Wordlist);
NWordlist=Shuffle(NWordlist);
...
%These will be the lure words (incorrect answers) for the long-term memory test
NWDistractorlist=fopen('NWDistractors.txt','r'); %Open the text file 
NWDistractorlist=textscan(NWDistractorlist,'%s'); %Reads the data from the file into a cell array
NWDistractorlist=cell2mat(NWDistractorlist); %Convert cell array into matrix
...
for i=0:b;  
  ...
  for t=1:n;
    ...
    if (wordcond==1)
      ...
      targText=NWordlist(1);
      %Create word list for presentation 
      newWords=[targText;NWordlist(2);NWordlist(3);NWordlist(4)]; %Creates array of the target word and three other words from the list
      arrayNums=[1:4]; %Number of different positions: top left, top right, bottom left, bottom right; 1 will always be the target word
      arrayNums=Shuffle(arrayNums); %Shuffle positions
    endif
    ...
    if (i>0)
      targetwords=[targetwords;targText,cuecond,wordcond];
    endif
    ...
    if (cuecond==1) %Display cue before stimuli
      ...
      Screen('FillRect',window,black);
      BlankOn=Screen('Flip',window,CueOn+CueDur);
      ...
      Screen('Flip',window,BlankOn+BlankDur);
      ...
      DrawFormattedText(window,char(newWords(arrayNums(3))),xc-xres/4,yc+yres/4,white);
      DrawFormattedText(window,char(newWords(arrayNums(4))),xc-xres/4,yc-yres/4,white);
      Screen('Flip',window,CueOn+CueDur+BlankDur);
    endif
  ...
  if (i>0)
    ...
    fprintf(datafilepointer,'%02d %02d %02d %02d %02d %f %02d %02d %02d\n',subNo,i,t,cuecond,wordcond,respRT,resp,actualPos,acc);
  ...
  if (wordcond==1)
    ...
    usedNWords=[usedNWords;NWordlist(1:4)];
    %Remove the words from the longer list so there are no repeats
    NWordlist(1:4)=[];
  endif
 endfor
...
Screen('Flip',window);
checkKeycode=0;
while(sum(checkKeycode==KbName('7'))<1)
  [rTime,checkKey]=KbWait();
  checkKeycode=find(checkKey);
endwhile
...
Distractorlist=Shuffle([Distractorlist;NWDistractorlist]);


%Grab cue and word conditions
cueconditions=cell2mat(cuewordlist(:,2));
wordconditions=cell2mat(cuewordlist(:,3));
...
for LTMi=1:LTMb;  
  %Number of trials per block
  LTMn=(b*n)/LTMb; %Number of trials equal number of block * trials from earlier divided by number of blocks
 ...
 %Instruction screen
  ...
  for LTMt=1:LTMn+lureN
    ...
    while (sum(buttons)==0)
      ...
      DrawFormattedText(window,rate2,x4-26,y4,white);
      DrawFormattedText(window,rate2,x5-11,y4,white);
      DrawFormattedText(window,rate1,x8-15,y4,white);
    ...
    %Find cue and word condition from previous experiment
    if(trialtype<=.2)
      LTMcuecond=0;
      LTMacc=0;
      LTMwordcond=0;
      ...
      LTMwordcond=wordconditions(1);
    endif
      
    %Write data to participant file 
    fprintf(LTMdatafilepointer,'%02d %02d %02d %02d %02d %02d %02d %f %f\n',subNo,LTMi,LTMt,LTMcuecond,LTMwordcond,dec,LTMacc,LTMresp,LTMrespRT);
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   %End of block screen
  if(LTMi>0) %experimental blocks
    bnumText=num2str(LTMi);
    bText=num2str(LTMb);
    bEndText=['This is the end of block ',bnumText,' of ',bText,'.\nPlease take a short break to move around and rest your eyes. \nPress <space> when you are ready to continue.'];
    Screen('FillRect',window,grey);
    DrawFormattedText(window,bEndText,'center','center',black);
...
%Wait for '7' key press to end experiment.
Screen('TextSize',window,30);
