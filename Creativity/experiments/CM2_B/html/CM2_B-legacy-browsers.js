...
let expName = 'CM2_B';  // from the Builder filename that created this script
...
function experimentInit() {
  ...
  problem_conditions = ["math"]; // only want math pts now
...
function trials_problemLoopBegin(trials_problemLoopScheduler) {
  ...
  // Schedule all the trials in the trialList:
  trials_problem.forEach(function() {
...
function rei_scaleRoutineBegin(snapshot) {
  return function () {
    ...
    rei_scaleComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function problem_instructionsRoutineBegin(snapshot) {
  return function () {
    ...
    problem_instructionsComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function problem_solvingRoutineBegin(snapshot) {
  return function () {
    ...
    problem_solvingComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
