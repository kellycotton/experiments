...
let expName = 'WMB2.5';  // from the Builder filename that created this script
...
function trials_fssLoopBegin(trials_fssLoopScheduler) {
  ...
  for (const thisTrials_fs of trials_fss) {
...
function trials_vasLoopBegin(trials_vasLoopScheduler) {
  ...
  // Schedule all the trials in the trialList:
  for (const thisTrials_va of trials_vas) {
...
function trials_vas_endLoopBegin(trials_vas_endLoopScheduler) {
  ...
  // Schedule all the trials in the trialList:
  for (const thisTrials_vas_end of trials_vas_end) {
...
function questionsLoopBegin(questionsLoopScheduler) {
  ...
  for (const thisQuestion of questions) {
    ...
    questionsLoopScheduler.add(question_responseRoutineBegin(snapshot));
    questionsLoopScheduler.add(question_responseRoutineEachFrame(snapshot));
    questionsLoopScheduler.add(question_responseRoutineEnd(snapshot));
...
function FSSRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of FSSComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function question_responseRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of question_responseComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
